...
class genetic:
    ...
    def mat_to_vector(self, mat_pop_weights):
        ...
        for sol_idx in range(mat_pop_weights.shape[0]):
            ...
            for layer_idx in range(mat_pop_weights.shape[1]):
                vector_weights = numpy.reshape(
                    mat_pop_weights[sol_idx, layer_idx],
    ...
    def vector_to_mat(self, vector_pop_weights, mat_pop_weights):
        ...
        for sol_idx in range(mat_pop_weights.shape[0]):
            ...
            for layer_idx in range(mat_pop_weights.shape[1]):
                ...
                mat_layer_weights = numpy.reshape(
                    curr_vector, newshape=(mat_pop_weights[sol_idx, layer_idx].shape)
                )
            ...
            logger.info(
                "Mean and variance of offspring: {} {}".format(
                    mat_layer_weights[sol_idx, :].mean(),
                    mat_layer_weights[sol_idx, :].std(),
                )
            )
    ...
    def crossover(self, parents, offspring_size, crossoverType="onePoint"):
        ...
        if crossoverType == "onePoint":
            ...
            for k in range(offspring_size[0]):
                ...
                logger.info(
                    "Mean and Variance of offspring: {} {}".format(
                        offspring[k, :].mean(), offspring[k, :].std()
                    )
                )
            ...
            for k in range(offspring_size[0]):
                ...
                offspring[k, 0:crossover_point1] = parents[
                    parent1_idx, 0:crossover_point1
                ]
                offspring[k, crossover_point1:crossover_point2] = genes_vec2
                # The new offspring will have its second half of its genes taken from the second parent.
                offspring[k, crossover_point2:] = parents[
                    parent1_idx, crossover_point2:
                ]
                logger.info(
                    "Mean and Variance of offspring: {} {}".format(
    ...
    def mutation(self, offspring_crossover, mutation_percent, mutationType="adaptive"):
        logger.info(
            "mutationType:{}, mutation_rate:{}".format(mutationType, mutation_percent)
        )
        ...
        if mutationType == "uniform":
            ...
            for idx in range(offspring_crossover.shape[0]):
                ...
                offspring_crossover[idx, mutation_indices] = (
                ...
                logger.info(
                    "Mean and Variance of offspring: {} {}".format(
                        offspring_crossover[idx, mutation_indices].mean(),
                        offspring_crossover[idx, mutation_indices].std(),
                    )
                )
            ...
            self.mutateRate = self.mutateRate * math.exp(
                self.learningRate * numpy.random.uniform(-0.5, 0.5)
            )
            logger.info("Mutation Rate new: {}".format(self.mutateRate))
            for idx in range(offspring_crossover.shape[0]):
                mutation = (self.mutateRate) * (numpy.random.normal(0, 0.5))
                offspring_crossover[idx, mutation_indices] = (
                    offspring_crossover[idx, mutation_indices] + mutation
                )
                # print('Mean and Variance of offspring: {} {}'.format(offspring_crossover[idx,mutation_indices].mean(), offspring_crossover[idx,mutation_indices].std()))
                logger.info(
                    "Mean and Variance of offspring: {} {}".format(
